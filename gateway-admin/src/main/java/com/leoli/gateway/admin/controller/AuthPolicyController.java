package com.leoli.gateway.admin.controller;

import com.leoli.gateway.admin.dto.ApiResponse;
import com.leoli.gateway.admin.model.AuthPolicyDefinition;
import com.leoli.gateway.admin.model.AuthPolicyEntity;
import com.leoli.gateway.admin.model.RouteAuthBindingEntity;
import com.leoli.gateway.admin.service.AuthPolicyService;
import lombok.RequiredArgsConstructor;
import lombok.extern.slf4j.Slf4j;
import org.springframework.http.ResponseEntity;
import org.springframework.web.bind.annotation.*;

import java.util.HashMap;
import java.util.List;
import java.util.Map;

/**
 * Authentication Policy Management Controller.
 * Provides CRUD operations for auth policies and route bindings.
 * Uses ApiResponse for standardized response format.
 *
 * @author leoli
 */
@Slf4j
@RestController
@RequestMapping("/api/auth")
@RequiredArgsConstructor
public class AuthPolicyController extends BaseController {

    private final AuthPolicyService authPolicyService;

    // ==================== Policy CRUD ====================

    /**
     * Get all auth policies.
     * @param instanceId Optional instance ID to filter policies
     */
    @GetMapping("/policies")
    public ResponseEntity<ApiResponse<List<AuthPolicyDefinition>>> getAllPolicies(
            @RequestParam(required = false) String instanceId) {
        List<AuthPolicyDefinition> policies;
        if (instanceId != null && !instanceId.isEmpty()) {
            policies = authPolicyService.getAllPolicies(instanceId);
        } else {
            policies = authPolicyService.getAllPolicies();
        }
        return ResponseEntity.ok(ApiResponse.success(policies));
    }

    /**
     * Get policy by ID.
     * @param policyId Policy ID
     * @param instanceId Optional instance ID
     */
    @GetMapping("/policies/{policyId}")
    public ResponseEntity<ApiResponse<AuthPolicyDefinition>> getPolicyById(
            @PathVariable String policyId,
            @RequestParam(required = false) String instanceId) {
        AuthPolicyDefinition policy;
        if (instanceId != null && !instanceId.isEmpty()) {
            policy = authPolicyService.getPolicy(instanceId, policyId);
        } else {
            policy = authPolicyService.getPolicy(policyId);
        }
        if (policy == null) {
            return ResponseEntity.status(404).body(ApiResponse.notFound("Policy not found: " + policyId));
        }
        return ResponseEntity.ok(ApiResponse.success(policy));
    }

    /**
     * Get policies by auth type.
     * @param authType Auth type
     * @param instanceId Optional instance ID to filter policies
     */
    @GetMapping("/policies/type/{authType}")
    public ResponseEntity<ApiResponse<List<AuthPolicyDefinition>>> getPoliciesByType(
            @PathVariable String authType,
            @RequestParam(required = false) String instanceId) {
        List<AuthPolicyDefinition> policies;
        if (instanceId != null && !instanceId.isEmpty()) {
            policies = authPolicyService.getPoliciesByType(instanceId, authType.toUpperCase());
        } else {
            policies = authPolicyService.getPoliciesByType(authType.toUpperCase());
        }
        return ResponseEntity.ok(ApiResponse.success(policies));
    }

    /**
     * Create a new auth policy.
     * @param policy Policy definition
     * @param instanceId Optional instance ID
     */
    @PostMapping("/policies")
    public ResponseEntity<ApiResponse<AuthPolicyDefinition>> createPolicy(
            @RequestBody AuthPolicyDefinition policy,
            @RequestParam(required = false) String instanceId) {
        try {
            log.info("Creating auth policy: {} (instance: {})", policy.getPolicyName(), instanceId);
            AuthPolicyEntity entity = authPolicyService.createPolicy(instanceId, policy);
            return ResponseEntity.ok(ApiResponse.success(authPolicyService.getPolicy(entity.getPolicyId()), "Policy created successfully"));
        } catch (IllegalArgumentException e) {
            log.warn("Failed to create policy: {}", e.getMessage());
            return ResponseEntity.badRequest().body(ApiResponse.badRequest(e.getMessage()));
        } catch (Exception e) {
            log.error("Failed to create policy", e);
            return ResponseEntity.internalServerError().body(ApiResponse.error("Failed to create policy: " + e.getMessage()));
        }
    }

    /**
     * Update an auth policy.
     */
    @PutMapping("/policies/{policyId}")
    public ResponseEntity<ApiResponse<AuthPolicyDefinition>> updatePolicy(
            @PathVariable String policyId,
            @RequestBody AuthPolicyDefinition policy) {
        try {
            log.info("Updating auth policy: {}, apiKey: {}, apiKeyHeader: {}", policyId, policy.getApiKey(), policy.getApiKeyHeader());
            authPolicyService.updatePolicy(policyId, policy);
            return ResponseEntity.ok(ApiResponse.success(authPolicyService.getPolicy(policyId), "Policy updated successfully"));
        } catch (IllegalArgumentException e) {
            log.warn("Failed to update policy: {}", e.getMessage());
            return ResponseEntity.status(404).body(ApiResponse.notFound(e.getMessage()));
        } catch (Exception e) {
            log.error("Failed to update policy", e);
            return ResponseEntity.internalServerError().body(ApiResponse.error("Failed to update policy: " + e.getMessage()));
        }
    }

    /**
     * Delete an auth policy.
     */
    @DeleteMapping("/policies/{policyId}")
    public ResponseEntity<ApiResponse<Void>> deletePolicy(@PathVariable String policyId) {
        try {
            log.info("Deleting auth policy: {}", policyId);
            authPolicyService.deletePolicy(policyId);
            return ResponseEntity.ok(ApiResponse.success("Policy deleted successfully"));
        } catch (IllegalArgumentException e) {
            log.warn("Failed to delete policy: {}", e.getMessage());
            return ResponseEntity.status(404).body(ApiResponse.notFound(e.getMessage()));
        } catch (Exception e) {
            log.error("Failed to delete policy", e);
            return ResponseEntity.internalServerError().body(ApiResponse.error("Failed to delete policy: " + e.getMessage()));
        }
    }

    /**
     * Enable an auth policy.
     */
    @PostMapping("/policies/{policyId}/enable")
    public ResponseEntity<ApiResponse<Void>> enablePolicy(@PathVariable String policyId) {
        try {
            log.info("Enabling auth policy: {}", policyId);
            authPolicyService.enablePolicy(policyId);
            return ResponseEntity.ok(ApiResponse.success("Policy enabled successfully"));
        } catch (IllegalArgumentException e) {
            log.warn("Failed to enable policy: {}", e.getMessage());
            return ResponseEntity.status(404).body(ApiResponse.notFound(e.getMessage()));
        } catch (Exception e) {
            log.error("Failed to enable policy", e);
            return ResponseEntity.internalServerError().body(ApiResponse.error("Failed to enable policy: " + e.getMessage()));
        }
    }

    /**
     * Get usage example for an auth policy.
     * Returns example headers, curl commands, and calculated values for testing.
     *
     * @param policyId Policy ID
     */
    @GetMapping("/policies/{policyId}/usage-example")
    public ResponseEntity<ApiResponse<Map<String, Object>>> getUsageExample(@PathVariable String policyId) {
        try {
            log.info("Getting usage example for auth policy: {}", policyId);
            Map<String, Object> example = authPolicyService.generateUsageExample(policyId);
            return ResponseEntity.ok(ApiResponse.success(example));
        } catch (IllegalArgumentException e) {
            log.warn("Failed to get usage example: {}", e.getMessage());
            return ResponseEntity.status(404).body(ApiResponse.notFound(e.getMessage()));
        } catch (Exception e) {
            log.error("Failed to get usage example", e);
            return ResponseEntity.internalServerError().body(ApiResponse.error("Failed to get usage example: " + e.getMessage()));
        }
    }

    /**
     * Disable an auth policy.
     */
    @PostMapping("/policies/{policyId}/disable")
    public ResponseEntity<ApiResponse<Void>> disablePolicy(@PathVariable String policyId) {
        try {
            log.info("Disabling auth policy: {}", policyId);
            authPolicyService.disablePolicy(policyId);
            return ResponseEntity.ok(ApiResponse.success("Policy disabled successfully"));
        } catch (IllegalArgumentException e) {
            log.warn("Failed to disable policy: {}", e.getMessage());
            return ResponseEntity.status(404).body(ApiResponse.notFound(e.getMessage()));
        } catch (Exception e) {
            log.error("Failed to disable policy", e);
            return ResponseEntity.internalServerError().body(ApiResponse.error("Failed to disable policy: " + e.getMessage()));
        }
    }

    // ==================== Route Binding Management ====================

    /**
     * Get all bindings.
     */
    @GetMapping("/bindings")
    public ResponseEntity<ApiResponse<List<RouteAuthBindingEntity>>> getAllBindings() {
        List<RouteAuthBindingEntity> bindings = authPolicyService.getAllBindings();
        return ResponseEntity.ok(ApiResponse.success(bindings));
    }

    /**
     * Get bindings for a policy.
     */
    @GetMapping("/bindings/policy/{policyId}")
    public ResponseEntity<ApiResponse<List<RouteAuthBindingEntity>>> getBindingsForPolicy(@PathVariable String policyId) {
        List<RouteAuthBindingEntity> bindings = authPolicyService.getBindingsForPolicy(policyId);
        return ResponseEntity.ok(ApiResponse.success(bindings));
    }

    /**
     * Get bindings for a route.
     */
    @GetMapping("/bindings/route/{routeId}")
    public ResponseEntity<ApiResponse<List<RouteAuthBindingEntity>>> getBindingsForRoute(@PathVariable String routeId) {
        List<RouteAuthBindingEntity> bindings = authPolicyService.getBindingsForRoute(routeId);
        return ResponseEntity.ok(ApiResponse.success(bindings));
    }

    /**
     * Get policies for a route.
     */
    @GetMapping("/policies/route/{routeId}")
    public ResponseEntity<ApiResponse<List<AuthPolicyDefinition>>> getPoliciesForRoute(@PathVariable String routeId) {
        List<AuthPolicyDefinition> policies = authPolicyService.getPoliciesForRoute(routeId);
        return ResponseEntity.ok(ApiResponse.success(policies));
    }

    /**
     * Create a binding (bind policy to route).
     */
    @PostMapping("/bindings")
    public ResponseEntity<ApiResponse<RouteAuthBindingEntity>> createBinding(@RequestBody Map<String, Object> request) {
        try {
            String policyId = (String) request.get("policyId");
            String routeId = (String) request.get("routeId");
            Integer priority = request.get("priority") != null ? (Integer) request.get("priority") : 100;

            if (policyId == null || routeId == null) {
                return ResponseEntity.badRequest().body(ApiResponse.badRequest("policyId and routeId are required"));
            }

            log.info("Creating binding: policy {} -> route {}", policyId, routeId);
            RouteAuthBindingEntity binding = authPolicyService.bindPolicyToRoute(policyId, routeId, priority);
            return ResponseEntity.ok(ApiResponse.success(binding, "Binding created successfully"));
        } catch (IllegalArgumentException e) {
            log.warn("Failed to create binding: {}", e.getMessage());
            return ResponseEntity.badRequest().body(ApiResponse.badRequest(e.getMessage()));
        } catch (Exception e) {
            log.error("Failed to create binding", e);
            return ResponseEntity.internalServerError().body(ApiResponse.error("Failed to create binding: " + e.getMessage()));
        }
    }

    /**
     * Batch bind policies to a route.
     */
    @PostMapping("/bindings/batch")
    public ResponseEntity<ApiResponse<List<RouteAuthBindingEntity>>> batchBind(@RequestBody Map<String, Object> request) {
        try {
            String routeId = (String) request.get("routeId");
            @SuppressWarnings("unchecked")
            List<String> policyIds = (List<String>) request.get("policyIds");

            if (routeId == null || policyIds == null || policyIds.isEmpty()) {
                return ResponseEntity.badRequest().body(ApiResponse.badRequest("routeId and policyIds are required"));
            }

            log.info("Batch binding {} policies to route {}", policyIds.size(), routeId);
            List<RouteAuthBindingEntity> bindings = authPolicyService.batchBind(routeId, policyIds);
            return ResponseEntity.ok(ApiResponse.success(bindings, "Batch binding created successfully"));
        } catch (Exception e) {
            log.error("Failed to batch bind", e);
            return ResponseEntity.internalServerError().body(ApiResponse.error("Failed to batch bind: " + e.getMessage()));
        }
    }

    /**
     * Delete a binding by ID.
     */
    @DeleteMapping("/bindings/{bindingId}")
    public ResponseEntity<ApiResponse<Void>> deleteBinding(@PathVariable String bindingId) {
        try {
            log.info("Deleting binding: {}", bindingId);
            authPolicyService.deleteBinding(bindingId);
            return ResponseEntity.ok(ApiResponse.success("Binding deleted successfully"));
        } catch (IllegalArgumentException e) {
            log.warn("Failed to delete binding: {}", e.getMessage());
            return ResponseEntity.status(404).body(ApiResponse.notFound(e.getMessage()));
        } catch (Exception e) {
            log.error("Failed to delete binding", e);
            return ResponseEntity.internalServerError().body(ApiResponse.error("Failed to delete binding: " + e.getMessage()));
        }
    }

    /**
     * Unbind policy from route.
     */
    @DeleteMapping("/bindings/policy/{policyId}/route/{routeId}")
    public ResponseEntity<ApiResponse<Void>> unbindPolicyFromRoute(
            @PathVariable String policyId,
            @PathVariable String routeId) {
        try {
            log.info("Unbinding policy {} from route {}", policyId, routeId);
            authPolicyService.unbindPolicyFromRoute(policyId, routeId);
            return ResponseEntity.ok(ApiResponse.success("Binding deleted successfully"));
        } catch (Exception e) {
            log.error("Failed to unbind", e);
            return ResponseEntity.internalServerError().body(ApiResponse.error("Failed to unbind: " + e.getMessage()));
        }
    }

    /**
     * Enable a binding.
     */
    @PostMapping("/bindings/{bindingId}/enable")
    public ResponseEntity<ApiResponse<Void>> enableBinding(@PathVariable String bindingId) {
        try {
            log.info("Enabling binding: {}", bindingId);
            authPolicyService.enableBinding(bindingId);
            return ResponseEntity.ok(ApiResponse.success("Binding enabled successfully"));
        } catch (IllegalArgumentException e) {
            log.warn("Failed to enable binding: {}", e.getMessage());
            return ResponseEntity.status(404).body(ApiResponse.notFound(e.getMessage()));
        } catch (Exception e) {
            log.error("Failed to enable binding", e);
            return ResponseEntity.internalServerError().body(ApiResponse.error("Failed to enable binding: " + e.getMessage()));
        }
    }

    /**
     * Disable a binding.
     */
    @PostMapping("/bindings/{bindingId}/disable")
    public ResponseEntity<ApiResponse<Void>> disableBinding(@PathVariable String bindingId) {
        try {
            log.info("Disabling binding: {}", bindingId);
            authPolicyService.disableBinding(bindingId);
            return ResponseEntity.ok(ApiResponse.success("Binding disabled successfully"));
        } catch (IllegalArgumentException e) {
            log.warn("Failed to disable binding: {}", e.getMessage());
            return ResponseEntity.status(404).body(ApiResponse.notFound(e.getMessage()));
        } catch (Exception e) {
            log.error("Failed to disable binding", e);
            return ResponseEntity.internalServerError().body(ApiResponse.error("Failed to disable binding: " + e.getMessage()));
        }
    }

    /**
     * Count bindings for a policy.
     */
    @GetMapping("/policies/{policyId}/bindings/count")
    public ResponseEntity<ApiResponse<Map<String, Object>>> countBindingsForPolicy(@PathVariable String policyId) {
        long count = authPolicyService.countBindingsForPolicy(policyId);
        Map<String, Object> data = new HashMap<>();
        data.put("policyId", policyId);
        data.put("bindingCount", count);
        return ResponseEntity.ok(ApiResponse.success(data));
    }
}